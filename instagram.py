...
class CommentInteractions:
        ...
        for _ in range(batches):
            ...
            for attempt in range(3):
                try:
                    ...
                    liked_count = self.like_comments_based_on_criteria(comments, liked_count, max_likes)

                    current_comments = set([comment[0] for comment in comments])
    ...
    def like_comment(self, comment_element):
        try:
            ...
            print(f"Error: The comment '{comment_element.text}' is not in the current view")

        except Exception:  # Catching general exceptions in case the like button isn't found or any other issues arise
            print(f"Error: Like button not found for the comment: '{comment_element.text}'.")
