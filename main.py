...
from instagram import InstagramActions, CommentInteractions  # Assuming you renamed the file after translating to UIautomator
...
try:
    ...
    # Reset processed comments at the start of each iteration
        ...
        for _ in range(5):  # Adjust the range as needed
            comment_data = comment_interactions.capture_comments(batches=4, liked_count=liked_so_far, max_likes=comments_to_like)
            print("Captured comments:", [comment[0] for comment in comment_data])
            ...
            liked_so_far += len([comment for comment in comment_data if comment[0] in comment_interactions.processed_comments])
            random_wait(2)
